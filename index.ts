...
export const MCP_RUNTIME_SNAPSHOT_EVENT =
  "pi-mcp-adapter:runtime-snapshot:v1" as const;
...
const runtimeRegistrars = new WeakMap<
  ExtensionAPI,
  (name: string, definition: ServerEntry) => McpServerRegistration
>();
const runtimeSnapshotters = new WeakMap<
  ExtensionAPI,
  (name: string) => McpRuntimeServerSnapshot
>();
...
function optionalNumber(options: {
  ...
  const number = (Type as { Number?: (opts: typeof options) => TSchema })
    .Number;
  // SAFETY: typebox is an optional peer dep; when absent the raw JSON-Schema object is returned — same wire shape TypeBox would produce
  return typeof number === "function"
    ? Type.Optional(number(options))
    : ({ type: "number", ...options } as unknown as TSchema);
}

function parseEnvDirectToolOverride(
...
function installMcpAdapter(pi: ExtensionAPI, options: McpAdapterOptions) {
  const sessionConfig =
    options.config === undefined ? undefined : cloneMcpConfig(options.config);
  ...
  function retainInitFailure(error: unknown): string {
    const message = truncateAtWord(
      formatTerminalError(error),
      INIT_FAILURE_MESSAGE_MAX_CHARS,
    );
  ...
  function buildInitRetryInstruction(
    prefix: string,
    failure: string | null = retainedInitFailure,
  ): string {
    const retry =
  ...
  function isOwnerAbortError(error: unknown, owner: McpRuntimeOwner): boolean {
    ...
    if (
      ...
      reason.message === "MCP initialization failed" &&
      error !== reason
    ) {
  ...
  function startGatewayRetryInitialization(ctx: ExtensionContext): void {
    ...
    startInitialization(
      ...
      oauthRuntime,
      generation,
      "stale_gateway_retry_initialization",
    );
  }
  ...
  const namespaceEnvOverride = resolveNamespaceEnvOverride(
    envRaw,
  ...
  const toolRenderShell =
    toolRenderOptions.resultRendering === "compact" ? "self" : "default";
  ...
  const runtimeServers = new Map<
    string,
    { definition: ServerEntry; entry: ServerEntry }
  >();
  ...
  function attachRuntimeServerLifecycle(
    targetState: McpExtensionState,
    name: string,
    definition: ServerEntry,
  ): void {
    const lifecycleMode = definition.lifecycle ?? "lazy";
    const persistsAfterFirstSpawn =
      lifecycleMode === "eager" || lifecycleMode === "lazy-keep-alive";
    const idleOverride =
      definition.idleTimeout ?? (persistsAfterFirstSpawn ? 0 : undefined);
    targetState.lifecycle.registerServer(
      name,
      definition,
      idleOverride === undefined ? undefined : { idleTimeout: idleOverride },
    );
    if (lifecycleMode === "keep-alive")
      targetState.lifecycle.markKeepAlive(name, definition);
  }
  ...
  const toToolParameters = (schema: Record<string, unknown>) =>
    typeof (Type as { Unsafe?: (value: never) => unknown }).Unsafe ===
    "function"
  ...
  function registerDirectTool(spec: DirectToolSpec, config: McpConfig): void {
    (pi.registerTool as (tool: unknown) => unknown)({
      ...
      parameters: toToolParameters(
        normalizeDirectToolInputSchema(spec.inputSchema),
      ),
      ...(config.settings?.strictDirectToolArguments === true
        ? {
            prepareArguments: (args: unknown) =>
              prepareDirectToolArguments(spec.inputSchema, args),
          }
        : {}),
  ...
  function activeFailureServers(): Set<string> {
    ...
    return new Set(
      Object.keys(currentState.config.mcpServers).filter((serverName) =>
        isServerInActiveFailureBackoff(currentState, serverName),
      ),
    );
  }

  function resolveCurrentDirectTools(
    config: McpConfig,
    ...
    return resolveDirectTools(
      config,
      cache,
      prefix,
      envDirectToolOverride,
      activeFailureServers(),
      reservedNames,
    );
  }
  ...
  function deactivateTools(toolNames: string[]): string[] {
    ...
    const unregisterTool = (
      pi as ExtensionAPI & { unregisterTool?: (name: string) => boolean }
    ).unregisterTool;
    const unregistered = toolNames.filter(
      (toolName) => unregisterTool?.(toolName) === true,
    );
    const fallbackNames = toolNames.filter(
      (toolName) => !unregistered.includes(toolName),
    );
  ...
  function applyDirectToolConfigChanges(
  ...
  function syncToolSurface(ctx?: ExtensionContext): void {
    ...
    syncNamespaceTools(
      config,
      cache,
      result.reservedDirectNames,
      result.activeDirectNames,
    );
    const changed =
      result.added.length + result.updated.length + result.deactivated.length;
  ...
  const getRuntimeServerSnapshot = (name: string): McpRuntimeServerSnapshot => {
    ...
    if (!runtimeServer) {
      throw new Error(
        `MCP runtime server "${name}" is not registered or has been disposed`,
      );
    }
    const activeState = state;
    if (!activeState) {
      throw new Error(
        `MCP runtime server "${name}" is unavailable because the adapter has no active state`,
      );
    }
    const activeEntry = activeState.config.mcpServers[name];
    if (
      Object.hasOwn(activeState.config.mcpServers, name) &&
      activeEntry !== runtimeServer.entry
    ) {
      throw new Error(
        `MCP runtime server "${name}" is shadowed by a configured server`,
      );
    }
    if (activeEntry !== runtimeServer.entry) {
      throw new Error(
        `MCP runtime server "${name}" is unavailable in the active adapter state`,
      );
    }
    return {
      name,
  ...
  pi.events.on(MCP_RUNTIME_REGISTER_EVENT, (rawRequest: unknown) => {
    ...
    if (request.version !== MCP_RUNTIME_REGISTER_VERSION) {
      request.result = {
        ok: false,
        error: new Error(
          `Unsupported MCP runtime registration version: ${String(request.version)}`,
        ),
      };
      return;
    }
    try {
      request.result = {
        ok: true,
        registration: registerRuntimeServer(request.name, request.definition),
      };
    } catch (error) {
      request.result = {
        ok: false,
  ...
  pi.events.on(MCP_RUNTIME_SNAPSHOT_EVENT, (rawRequest: unknown) => {
    ...
    if (request.version !== MCP_RUNTIME_SNAPSHOT_VERSION) {
      request.result = {
        ok: false,
        error: new Error(
          `Unsupported MCP runtime snapshot version: ${String(request.version)}`,
        ),
      };
      return;
    }
    try {
      request.result = {
        ok: true,
        snapshot: getRuntimeServerSnapshot(request.name),
      };
    } catch (error) {
      request.result = {
        ok: false,
    ...
    return promise
      .then(async (nextState) => {
        if (
          ...
          try {
            ...
            console.error(
              `MCP: failed to clean stale initialization state: ${formatTerminalError(error)}`,
            );
          }
          return;
        }
        ...
        for (const [name, { entry }] of runtimeServers) {
          if (Object.hasOwn(nextState.config.mcpServers, name)) {
            console.error(
              `MCP: runtime-registered server "${name}" now collides with a configured server; keeping the configured server`,
            );
            continue;
          }
        ...
        nextState.onToolMetadataUpdated = (_serverName, _reason) => {
          ...
          if (directToolsFrozen) {
            logger.debug(
              `MCP: metadata update for ${_serverName} (${_reason}) skipped — directTools frozen`,
            );
            return;
          }
          syncToolSurface(ctx);
      ...
      .catch(async (err) => {
        ...
        try {
          ...
          console.error(
            `MCP: failed to clean rejected initialization: ${formatTerminalError(error)}`,
          );
        }
      });
  }
  ...
  function startLoadTimeInitialization(): void {
    const hasStartupServer = Object.values(earlyConfig.mcpServers).some(
      (definition) => {
        if (definition.disabled === true) return false;
        return (
          definition.lifecycle === "eager" ||
          definition.lifecycle === "keep-alive"
        );
      },
    );
    ...
    setImmediate(() => {
      ...
      currentOwner = owner;
      currentOAuthRuntime = oauthRuntime;
      // SAFETY: minimal load-time ctx; only mode/cwd/signal are consumed on this non-UI initialization path
      startInitialization(
        {
          mode: "print",
          hasUI: false,
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    try {
      await Promise.all([
        ...
        previousOAuthRuntime
          ? shutdownOAuth(previousOAuthRuntime)
          : Promise.resolve(),
      ]);
    } catch (error) {
      console.error(
        `MCP: failed to shut down previous session state: ${formatTerminalError(error)}`,
      );
    }
  ...
  pi.on("input", async () => {
    ...
    try {
      ...
      if (!isAbortError(error, inputOwner.signal)) {
        logger.debug(
          `MCP: keep-alive convergence failed before input: ${formatTerminalError(error)}`,
        );
      }
    }
  });
  ...
  pi.on("session_shutdown", async () => {
    ...
    const stopOwner =
      owner?.stop("MCP extension session shutdown") ?? Promise.resolve();
    try {
      await Promise.all([
        stopOwner,
      ...
      console.error(
        `MCP: session shutdown cleanup failed: ${formatTerminalError(error)}`,
      );
    }
  });
  ...
  const registerMcpCommand = (commandName: string) =>
    pi.registerCommand(commandName, {
      ...
      getArgumentCompletions: (prefix: string) => {
        ...
        if (subcommand === "token") {
          const tokenMatch = argumentPrefix
            .trimStart()
            .match(/^(set|remove|status)\s+(.*)$/);
          if (!tokenMatch) {
            const actions = ["set", "remove", "status"]
              .filter((action) => action.startsWith(argumentPrefix.trimStart()))
              .map((action) => ({
                value: `token ${action} `,
                label: `${action} — Bearer token ${action}`,
              }));
          ...
          const servers = Object.keys(state.config.mcpServers)
            .filter((serverName) =>
              serverName.startsWith(serverPrefix.trimStart()),
            )
            .map((serverName) => ({
              value: `token ${action} ${serverName}`,
              label: serverName,
            }));
        ...
        const servers = Object.keys(state.config.mcpServers)
          .filter((serverName) =>
            serverName.startsWith(argumentPrefix.trimStart()),
          )
          .map((serverName) => ({
            value: `${subcommand} ${serverName}`,
            label: serverName,
          }));
        return servers.length > 0 ? servers : null;
      },
      handler: async (args, ctx) => {
        ...
        // SAFETY: commandCtx replicates ExtensionContext from the live ctx plus command-scoped owner state
        const commandCtx = {
          hasUI: commandHasUI,
          ui: commandHasUI
            ? commandOwner
        ...
        switch (subcommand) {
          ...
          case "token": {
            ...
            if (
              ...
              if (commandCtx.hasUI)
                commandCtx.ui?.notify(
                  "Usage: /mcp token set|remove|status <server>",
                  "error",
                );
              return;
            }
          ...
          case "enable": {
            ...
            if (!state.config.mcpServers[serverName]) {
              commandCtx.ui?.notify(
                `Server "${serverName}" not found in effective config`,
                "error",
              );
              break;
            }
  ...
  if (earlyConfig.settings?.scriptMode !== false) {
    (pi.registerTool as (tool: unknown) => unknown)({
      ...
      parameters: Type.Object({
        code: Type.String({
          description:
            "Trusted JavaScript MCP script. Use tools.<prefixedToolName>(args) and emit(value).",
        }),
      ...
      async execute(
        ...
        if (!state && initPromise) {
          try {
            ...
            if (executeOwner && isOwnerAbortError(error, executeOwner))
              throw error;
            const message =
              error instanceof Error ? error.message : String(error);
            return {
        ...
        if (!state) {
          return {
            content: [
              {
                type: "text" as const,
                text: retainedInitFailure
                  ? buildInitRetryInstruction(
                      "MCP is not initialized after an earlier initialization failure",
                    )
                  : "MCP not initialized",
              },
            ],
            details: {
              ...
              ...(retainedInitFailure ? { message: retainedInitFailure } : {}),
            },
          };
        }
        executeOwner?.throwIfInactive();
    ...
    (pi.registerTool as (tool: unknown) => unknown)({
      ...
      parameters: Type.Object({
        ...
        args: Type.Optional(
          Type.Union(
            [
              Type.String({
                description:
                  'Arguments as a JSON string (e.g., \'{"key": "value"}\')',
              }),
              Type.Object(
                {},
                {
                  ...
                  description:
                    'Arguments as a JSON object (e.g., { "key": "value" })',
                },
              ),
            ],
            {
              description:
                "Tool arguments as a JSON object, or as a JSON string encoding one",
            },
          ),
        ),
        ...
        action: Type.Optional(
          Type.String({
            description:
              "Action: 'ui-messages', 'auth-start', or 'auth-complete'",
          }),
        ),
      }),
      renderResult: renderMcpToolResult,
      async execute(
          ...
          try {
            ...
            return {
              content: [
                {
                  type: "text" as const,
                  text: buildInitRetryInstruction(
        ...
        if (params.action === "auth-complete") {
          ...
          const input =
            parsedArgs?.redirectUrl ?? parsedArgs?.code ?? parsedArgs?.input;
  ...
  function syncProxyTool(
    ...
    const missingConfiguredDirectToolServers =
      getMissingConfiguredDirectToolServers(
        config,
        cache,
        envRaw === undefined || envRaw === "__none__"
          ? undefined
          : envDirectToolOverride,
      );
  ...
  syncNamespaceTools(
    earlyConfig,
    earlyCache,
    initialDirectResult.reservedDirectNames,
    initialDirectResult.activeDirectNames,
  );
  ...
  return function mcpAdapter(pi: ExtensionAPI) {
    installMcpAdapter(pi, {
      ...
      ...(factoryConfig === undefined
        ? {}
        : { config: cloneMcpConfig(factoryConfig) }),
    });
  };
}
 ...
 */
export function getRuntimeMcpServerSnapshot(options: {
  pi: ExtensionAPI;
  name: string;
