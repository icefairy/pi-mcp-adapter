...
### Fork notice (@icefairy/pi-mcp-adapter)

This fork of upstream [`pi-mcp-adapter`](https://github.com/nicobailon/pi-mcp-adapter) v2.29.0 intentionally differs from the original:

- **Intranet HTTP OAuth**: patches `@modelcontextprotocol/client` so OAuth token endpoints over plain HTTP are accepted for private intranet MCP servers (upstream requires HTTPS except loopback). Applied by a self-contained idempotent postinstall script (`scripts/apply-intranet-http-patch.mjs`).
- **Install size**: reduced from ~481 MB to ~50 MB by moving `@earendil-works/pi-coding-agent` / `pi-ai` / `pi-tui` / `typebox` to optional peer dependencies (types only), moving `recheck` to optionalDependencies, and removing the unused `ext-apps` dependency.
- **Package name / OAuth clientInfo**: renamed to `@icefairy/pi-mcp-adapter`.

Everything else matches upstream v2.29.0.

### Fixed

- **2.29.2 regression**: the published tarball omitted `patches/` and relied on `patch-package`, which could not locate the hoisted SDK when running as a dependency — the intranet-HTTP OAuth patch silently never applied for npm users. Replaced with a self-contained idempotent postinstall script that locates `@modelcontextprotocol/client` wherever npm hoists it.

### Changed

- Codebase formatted with Biome (line-wrapping and statement style only; no behavior changes).
- Added `SAFETY:` annotations at the few intentional `as unknown as` boundary casts to satisfy the lint pipeline; no runtime changes.
