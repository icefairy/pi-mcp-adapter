...
type AuthDiscovery = Pick<
  AuthOptions,
  "resourceMetadataUrl" | "scope" | "skipIssuerMetadataValidation"
>;
...
export function hasPendingAuth(
  ...
  return Array.from(state.pendingAuths.values()).some(
    (pendingAuth) => pendingAuth.serverName === serverName,
  );
}
...
export function extractOAuthConfig(definition: ServerEntry): McpOAuthConfig {
  ...
  if (definition.oauth?.skipIssuerMetadataValidation !== undefined) {
    ...
    config.skipIssuerMetadataValidation =
      definition.oauth.skipIssuerMetadataValidation;
  }
  return config;
}
...
async function probeAuthDiscovery(
  ...
  const discoveryHeaders = definition?.headers
    ? Object.fromEntries(
        Object.entries(definition.headers).filter(
...
export async function startAuth(
  ...
  if (config.grantType === "client_credentials") {
    const storedAuth = await getAuthForUrl(
      serverName,
      serverUrl,
      authStorageOptions,
    );
    ...
    const authProvider = new McpOAuthProvider(
      ...
      config,
      {
        onRedirect: async () => {
          throw new Error(
            "Browser redirect is not used for client_credentials flow",
          );
        },
      },
      authStorageOptions,
    ...
    try {
      const discovery = applyOAuthConfig(
        await probeAuthDiscovery(serverUrl, definition, signal),
        config,
      );
      throwIfAborted(signal);
      const result = await abortable(
        runSdkAuth(authProvider, { serverUrl, ...discovery }),
        signal,
      );
      throwIfAborted(signal);
  ...
  const existingPendingAuth = runtimeState.pendingAuths.get(
    getPendingAuthKey(serverName, authStorageOptions),
  );
  ...
  try {
    ...
    authProvider.deactivate();
    try {
      await clearPendingAuthAndReleaseIfIdle(
        runtime,
        serverName,
        oauthState,
        authStorageOptions,
...
async function clearPendingAuth(
  ...
  const authStorageOptions =
    pendingAuth?.authStorageOptions ?? fallbackStorageOptions;
...
async function cleanupAndReleaseCallbackServerIfIdle(
  ...
  try {
    ...
    if (cleanupFailure) {
      throw new AggregateError(
        [cleanupFailure.error, releaseError],
 ...
 */
export function parseAuthorizationRedirectInput(
  input: string,
...
export async function waitForAuthorizationResponse(
  ...
  try {
    const response = await abortable(
      Promise.race([
        callbackPromise.then((input) => ({
          input,
          source: "callback" as const,
        })),
        onAuthorizationInput(authorizationUrl, inputController.signal).then(
          (input) => ({
            input,
...
export async function completeAuth(
  ...
  const authStorageOptions =
    pendingAuth?.authStorageOptions ?? fallbackAuthStorageOptions;
  if (!pendingAuth) {
  ...
  try {
    ...
    const expectedIssuer =
      metadata?.issuer ?? discoveryState?.authorizationServerUrl;
      ...
      try {
        ...
        if (caughtError !== undefined) {
          throw new AggregateError(
            ...
            "OAuth completion cleanup failed",
          );
        }
        throw cleanupError;
      }
...
export async function authenticate(
  ...
  const operation = (async (): Promise<AuthStatus> => {
    ...
    try {
      ...
      oauthState = runtimeState.pendingAuthStates.get(
        getPendingAuthKey(serverName, authStorageOptions),
      );
      throwIfAborted(signal);
      if (!oauthState) {
      ...
      try {
        ...
        throw new AggregateError(
          [error, cleanupError],
          "OAuth cancellation cleanup failed",
        );
      }
      throw error;
    }
...
export async function getValidToken(
  ...
  if (expired === true && entry.tokens.refreshToken) {
    console.log(
      `MCP Auth: Token expired for ${serverName}, attempting refresh`,
    );
    ...
    try {
      ...
      try {
        ...
        const discovery = await probeAuthDiscovery(
          serverUrl,
          undefined,
          signal,
        );
        ...
        const refreshed = await getAuthForUrl(
          serverName,
          serverUrl,
          authStorageOptions,
        );
      ...
      if (
        ...
        error instanceof OAuthCredentialStoreError
      )
        throw error;
...
export async function initializeOAuth(
  ...
  legacyRuntime = createOAuthRuntime(
    runtimeOrSignal as AbortSignal | undefined,
  );
  return legacyRuntime;
}
...
export async function shutdownOAuth(
  runtime: McpOAuthRuntime = legacyRuntime,
): Promise<void> {
  const state = getRuntimeState(runtime);
