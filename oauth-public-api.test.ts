...
} = await import("@icefairy/pi-mcp-adapter/oauth");
const { getAuthForUrl, resetTestAuthSecretStore, saveAuthEntry } = await import(
  "./mcp-auth.ts"
);
...
describe("public OAuth token API", () => {
  ...
  it("reads and updates URL-bound tokens through the package export", async () => {
    ...
    assert.deepStrictEqual(
      await getMcpOAuthTokensForUrl("jira", "https://jira.example.com/mcp"),
      {
        accessToken: "access-1",
        refreshToken: "refresh-1",
        expiresAt,
    ...
    assert.deepStrictEqual(
      inspectMcpOAuthTokensForUrl("jira", "https://jira.example.com/mcp"),
      {
        status: "present",
        tokens: {
          accessToken: "access-1",
  ...
  it("does not return tokens for a different URL", async () => {
    ...
    assert.strictEqual(
      await getMcpOAuthTokensForUrl("jira", "https://other.example.com/mcp"),
      undefined,
    );
    assert.deepStrictEqual(
      inspectMcpOAuthTokensForUrl("jira", "https://other.example.com/mcp"),
      {
        status: "absent",
      },
    );
  ...
  it("does not expose client info or OAuth flow secrets", () => {
    ...
    const status = inspectMcpOAuthTokensForUrl(
      "jira",
      "https://jira.example.com/mcp",
    );
  ...
  it("clears stale URL-bound state when tokens move to another URL", async () => {
    ...
    updateMcpOAuthTokensForUrl("jira", "https://new.example.com/mcp", {
      accessToken: "new-token",
    });

    assert.strictEqual(
      await getMcpOAuthTokensForUrl("jira", "https://old.example.com/mcp"),
      undefined,
    );
    assert.deepStrictEqual(
      await getMcpOAuthTokensForUrl("jira", "https://new.example.com/mcp"),
      {
        accessToken: "new-token",
      },
    );
